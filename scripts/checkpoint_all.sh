...
SPIKE="${PROJECT_DIR}/riscv-isa-sim/install/bin/spike"
...
echo "=== Batch checkpoint: $TOTAL ELFs (${JOBS} parallel) ==="
echo "  spike: $SPIKE"
echo "  pk:    $PK"
...
export SPIKE PK ISA COMPRESS_FLAG CKPT_ELF_DIR CKPT_OUT_DIR
...
run_one() {
    ...
    if $SPIKE $ISA $COMPRESS_FLAG --save="$ckpt_path" "$PK" "$elf" </dev/null >/dev/null 2>&1; then
...
find "$CKPT_ELF_DIR" -type f -executable | sort | \
    ...
    echo "" >&2
    echo "=== Done: $OK OK, $FAIL FAIL (total $TOTAL) ===" >&2
    if [[ $FAIL -gt 0 ]]; then
        echo "Failed:" >&2
        echo -e "$FAIL_LIST" >&2
        exit 1
