/* Dependency test: LD↔DIV (L2) forwarding chain */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
extern void test_ld_div(uint64_t nloop, void *head);
...
int main(void)
{
    ...
    test_ld_div(1000, chain);
    perf_start(&ctx);
    test_ld_div(NLOOP, chain);
    perf_stop(&ctx, &cycles, &instret);
    perf_print("LD<->DIV L2", cycles, instret,
              (uint64_t)NLOOP * 20, 2);
