/* Dependency test: LD→LD (L2) pointer chase baseline */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
extern void test_ld_ld(uint64_t nloop, void *head);
...
int main(void)
{
    ...
    test_ld_ld(1000, chain);
    perf_start(&ctx);
    test_ld_ld(NLOOP, chain);
    perf_stop(&ctx, &cycles, &instret);
    perf_print("LD->LD L2", cycles, instret,
