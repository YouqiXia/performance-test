/* Dependency test: LD↔ALU (L1) forwarding chain */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
extern void test_ld_alu(uint64_t nloop, uint64_t *node);
...
int main(void)
{
    ...
    test_ld_alu(1000, node);
    perf_start(&ctx);
    test_ld_alu(NLOOP, node);
    perf_stop(&ctx, &cycles, &instret);
    perf_print("LD<->ALU L1", cycles, instret,
