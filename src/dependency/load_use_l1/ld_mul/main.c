/* Dependency test: LD↔MUL (L1) forwarding chain */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
extern void test_ld_mul(uint64_t nloop, uint64_t *node);
...
int main(void)
{
    ...
    test_ld_mul(1000, node);
    perf_start(&ctx);
    test_ld_mul(NLOOP, node);
    perf_stop(&ctx, &cycles, &instret);
    perf_print("LD<->MUL L1", cycles, instret,
